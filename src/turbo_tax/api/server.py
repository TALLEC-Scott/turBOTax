...
VAULT_PATH = Path(os.getenv("OBSIDIAN_VAULT", "/udir/stallec/home/turbo-tax/obsidian_db/turbo_tax"))
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "paths": {"type": "array", "items": {"type": "string"}, "description": "Array of note paths (max 10)"}
                },
                "required": ["paths"]
            }
        }
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "depth": {"type": "integer", "description": "Recursion depth for backlinks of backlinks (default 1, max 3)", "default": 1}
                },
                "required": ["path"]
            }
        }
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "path": {"type": "string", "description": "Path to the note to find outlinks for"},
                    "depth": {"type": "integer", "description": "Recursion depth for following links (default 1, max 3)", "default": 1}
                },
                "required": ["path"]
            }
        }
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "depth": {"type": "integer", "description": "Recursion depth (default 2, max 3)", "default": 2},
                    "max_notes": {"type": "integer", "description": "Maximum notes to return (default 20)", "default": 20}
                },
                "required": ["path"]
            }
        }
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "path": {"type": "string", "description": "Path to the index file (e.g., '03 - Topics/_index.md')"},
                    "max_notes": {"type": "integer", "description": "Maximum notes to crawl (default 10, max 20)", "default": 10},
                    "filter_links": {"type": "array", "items": {"type": "string"}, "description": "Optional: only follow these specific link names"}
                },
                "required": ["path"]
            }
        }
    ...
    for line in parts[1].strip().split("\n"):
        if ":" in line:
            ...
            if value.startswith("[") and value.endswith("]"):
                ...
                frontmatter[key.strip()] = [i.strip().strip('"') for i in items if i.strip()]
            else:
...
def tool_list_directory(path: str = "/") -> dict:
    ...
    items = [{"name": i.name, "type": "directory" if i.is_dir() else "file", "path": str(i.relative_to(VAULT_PATH))} 
                ...
                if target.lower() in links or target.lower().replace(' ', '-') in links:
                    ...
                    if rel_path.lower() not in visited:
                        ...
                        if current_depth < depth:
                            deeper = find_backlinks_recursive(md_file.stem, current_depth + 1)
                            found.extend(deeper)
            except Exception:
                continue
            ...
            if linked_note:
                ...
                if linked_rel_path.lower() not in visited:
                    try:
                        ...
                        # Recurse
                        if current_depth < depth:
                            deeper = find_outlinks_recursive(linked_note, current_depth + 1)
                            found.extend(deeper)
                    except Exception:
                        continue
    ...
    for md_file in VAULT_PATH.rglob("*.md"):
        try:
            ...
            if fm.get("type") == "index":
                ...
                if path.lower() in str(md_file).lower() or path.lower() in md_file.parent.name.lower():
                    return md_file
        except Exception:
            continue
...
def tool_crawl_from_index(path: str, max_notes: int = 10, filter_links: list[str] | None = None) -> dict:
...
async def generate_response(
    ...
    for iteration in range(max_iterations):
        ...
        logger.info(f"📝 Final response: {len(content)} chars, finish_reason: {choice.get('finish_reason')}")
        if content:
...
async def chat(request: ChatRequest):
    ...
    async def event_stream():
        async with httpx.AsyncClient(timeout=120.0) as client:
            async for chunk in generate_response(request.message, client):
                yield chunk
        ...
        try:
            ...
            for match in link_pattern.finditer(content):
                ...
                for target_path, target_id in node_map.items():
                    if target_name in target_path or target_path.endswith(f"{target_name}.md"):
                        edges.append({
